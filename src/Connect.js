...
const Connect = () => {
  ...
  const [RFIDTagID, setRFIDTagID] = useState(null);
  ...
  const handleCharacteristicValueChanged = (event) => {
    setRFIDTagID(event.target.value.getUint8(0) + '%');
  }
  ...
  const connectToDeviceAndSubscribeToUpdates = async () => {
    try {
      ...
      setRFIDTagID(reading.getUint8(0) + '%');
    } catch(error) {
  ...
  return (
    <div className="connection-main">
      <h1>Connect to Your Smart Basket Over Bluetooth</h1>
      <br></br>
